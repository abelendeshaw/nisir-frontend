// Data for the seven Nisir Designs capability pages. Centralizing this here
// means the home page list, header dropdown, footer links, and each
// capability's own page all render from one source instead of seven
...
const dimensionalFields: CapabilityField[] = [
  { type: "dimensions" },
...
export function capabilityName(capability: Pick<Capability, "titleTop" | "titleBottom">) {
  return `${capability.titleTop} ${capability.titleBottom}`;
}
...
export function nextCapability(slug: string) {
  const index = capabilities.findIndex((capability) => capability.slug === slug);
  return capabilities[(index + 1) % capabilities.length];
}
